...
FREE_UPSELL_TEXTS = {
    ...
    "love": (
        ...
        "• какой партнёр подходит: где возможна встреча, внешность и характер;\n"
        "• с кем может быть сложно, что усиливает отношения и итоговый любовный портрет.\n\n"
    ...
    "compatibility": (
        ...
        "• какой партнёр тебе подходит: где возможна встреча, внешность и характер;\n"
...
PAID_OFFER_TEXTS = {
    ...
    "love": (
        ...
        "ревность и дистанция, конфликты, сценарии отношений, портрет подходящего "
        "партнёра (встреча, внешность, характер) и итоговый любовный портрет."
    ),
    "compatibility": (
        "Полный разбор совместимости по данным вашей пары.\n\n"
        "В PDF — динамика, общение, доверие, границы, конфликты, что притягивает "
        "и раздражает, потребности каждого, портрет подходящего партнёра "
        "(встреча, внешность, характер) и итоговый портрет пары."
    ),
    "money": (
...
SCENARIO_INTROS = {
    ...
    "love": (
        ...
        "Полный PDF раскроет близость, конфликты, сценарии и портрет подходящего партнёра."
    ),
    "compatibility": (
        "💑 СОВМЕСТИМОСТЬ\n\n"
...
def models_settings_message(models: dict[str, str] | None = None) -> str:
    lines = [
        ...
        "• Скелет PDF — каркас разделов (2–3 пункта) и короткая задумка",
        "• Разделы PDF — готовый текст каждого раздела по скелету",
        "",
...
def admin_text(
    *,
    ...
    return (
        ...
        "Раздел «Статистика» — заказы, заходы и ссылки.\n"
        ...
        "Раздел «Промпты» — системные правила, инструкции продуктов и сборка запроса."
    )
