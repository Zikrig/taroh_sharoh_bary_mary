...
def _load_sections() -> dict[str, list[tuple[str, str]]]:
    ...
    sources = {
        "personality": root / "1.csv",
        "compatibility": root / "2.csv",
        "money": root / "3.csv",
    }
    sections: dict[str, list[tuple[str, str]]] = {}
    for report_type, path in sources.items():
        with path.open(encoding="utf-8-sig", newline="") as source:
            rows = list(csv.DictReader(source))
        if not rows or any(not row.get("Раздел") or not row.get("Содержание") for row in rows):
            raise ValueError(f"Некорректная структура разделов в {path}.")
        sections[report_type] = [
            (row["Раздел"].strip(), row["Содержание"].strip())
            for row in rows
        ]
    return sections
